...
use super::pi::{pi_extension_path, pi_extension_version};
use super::*;
...
async fn install_then_uninstall_pi_extension() {
    ...
    assert!(extension.contains("void postHook("));
    assert!(!extension.contains("await postHook("));
    assert!(extension.contains("AbortController"));
    assert!(extension.contains("REQUEST_TIMEOUT_MS = 2_000"));
...
#[tokio::test]
// 验证旧的托管 Pi 扩展会在原位升级到当前模板版本。
async fn managed_legacy_pi_extension_is_upgraded_in_place() {
    let tmp = TempDir::new().unwrap();
    let pi_dir = tmp.path().join("pi-agent");
...
fn future_managed_pi_extension_is_not_downgraded() {
    ...
    fs::create_dir_all(&extensions_dir).unwrap();
    let extension_path = pi_extension_path(&pi_dir);
    let future_version = PI_EXTENSION_VERSION + 1;
    let future = format!(
        "// {PI_EXTENSION_MARKER}\n// {PI_EXTENSION_VERSION_PREFIX}{future_version}\n// {PI_MODULE_SESSION_START}\n// {PI_MODULE_RUNNING}\n// {PI_MODULE_STOP}\nconst source = \"pi\";\n"
    );

...
### Agent 能力隐藏 MCP 健康状态展示

- 侧边栏「实时统计 → Agent 能力」卡片不再展示 MCP 的正常/异常/健康未知分布；详情弹窗的 MCP 行不再显示健康徽章与“无可验证证据”说明，MCP 过滤器同步移除三个健康选项。探测不到 MCP 健康状态时不再把“健康未知”当作结论展示。
- 仅隐藏前端展示：MCP 激活/禁用徽章、Skills 状态、刷新与深度检查、会话 ID、采集时间、证据时间与错误码全部保留；后端诊断、证据合并与快照字段不变，i18n 文案保留，需要时可按原样恢复渲染。
...
- 修复 Web 文件树首次展开目录仍需等待桌面端固定轮询的问题：收到后端操作事件后立即领取任务，事件突发或执行中到达时合并补跑，保留 1 秒兜底轮询；快速目录读取不再闪烁加载图标。
...
- 桌面文件树复制／剪切同步写入 Windows 系统文件剪贴板（CF_HDROP 与复制／移动效果），可在应用内或资源管理器粘贴；保留项目代际和系统剪贴板变更校验，写入失败明确提示只保留应用内副本。WSL 项目不伪造本地文件路径。
...
- 第三方 Pi 扩展（permission-gate 的危险命令确认、confirm-destructive 等）的对话框现在可以在宠物端直接回答：`select` / `confirm` / `input` 的选项会搬到宠物端与终端竞速，谁先答谁生效，另一端自动收起或清除待处理项。代答走待处理项通道（受「宠物端问题交互」开关管），不再依赖通知气泡，所以关掉「通知提醒」也能在宠物端看到并处理。
- 对话框文本带警告标记或命中不可逆命令（`rm -rf`、`sudo`、`chmod 777`、`git reset --hard`、`git push --force`、`DROP TABLE` 等）时，宠物端把「放行」那一项标红；不加二次确认，一次点击即生效。
- 宠物端接不住时自动退回原来的行为：未启用宠物端交互、宠物未运行，或选项不适配（`editor` / `custom` 对话框、选项超过 64 项、选项文本重复）时，只镜像成一条「终端里有对话框在等」提醒，回答仍在终端完成。
- 修复 Pi 的问题组在终端里丢失选项说明与推荐理由的问题：接管 `question` / `questionnaire` 工具时，参数 schema 补回了与 Pi 原生扩展一致的字段说明，模型重新为每个选项生成解释；此前 schema 省略了这些说明，模型只给出光秃秃的选项标签。
- Pi 的问题组在终端一侧改为自绘面板，与 Pi 原生 `question` / `questionnaire` 的呈现一致：选项带编号、说明另起一行缩进显示，多问题时顶部有 Tab 条与提交页并汇总已选答案，自定义答案走面板内嵌的编辑器而不再弹第二个对话框。宠物端与终端的竞速、抢先收起、待处理项清除行为不变。
...
- 修复宠物端输入自定义答案时被定时刷新打断的问题：快照刷新重建 DOM 时除滚动位置外，同时保留焦点元素与文本光标位置，打字过程不再中断。
- Pi 的问题组与权限确认改为宠物端与终端竞速并存：两端同时显示，谁先回答谁生效，另一端自动收起或清除待处理项，不再只能在宠物端作答。
- 会话端答完后宠物端待处理项自动消除：Claude/Grok 的 `PostToolUse`（ToolStop/AgentToolStop）按同一 `toolUseId` 精确清除，Pi 由扩展在竞速结束时向 broker 发取消。Codex hook 协议不含 PostToolUse，其 hook 链路仍需等 Stop；Codex app-server 链路本身是互斥转发，不受影响。
- 第三方 Pi 扩展（如 permission-gate 的危险命令确认）直接调用 Pi 原生对话框，宠物端无法代答，现镜像为一条宠物端提醒告知「终端里有对话框在等待」，回答仍在终端内完成。Pi 0.84.4+ 优先用官方 `ui_prompt_start` / `ui_prompt_end` 事件（额外覆盖 `editor` 与 `custom` 对话框），较旧版本仍靠包裹 `ctx.ui` 兜底。
- 已关闭会话在宠物端遗留的完成/失败状态 30 秒后自动消除，不必再手动点「清除」；超时前仍可手动清除，会话被恢复则重新计时。
...
- 新增由 CLI-Manager 内置管理的 Windows Electron 桌面宠物E，复用已校验的本机 Codex Pets 精灵包，提供四色任务状态、通知气泡、所属会话跳转，以及 Claude、Codex、Pi、Grok 可验证能力的问题与审批交互；与原桌面宠物保持运行时互斥。
- 桌面宠物E设置中的宠物选择改为静态图像预览网格，显示宠物名称和当前选中状态；首次自动选择与手动选择共享同一写入锁，重新扫描可恢复此前加载失败的预览。
- 修复 Windows 安装资源路径带 `\\?\` 或 `\\?\UNC\` verbatim 前缀时 Electron 误解析应用入口、导致宠物E无法启动的问题；运行前只规范化进程参数，安装包校验仍使用原始资源路径。
- 修复 Windows Electron GUI 进程启动后标准输入立即结束、宠物E只发送 `hello` 就退出的问题；CLI-Manager 现在通过每实例随机、本机专用且拒绝远程客户端的命名管道发送原 JSON Lines 状态协议，不新增监听端口，子进程诊断仍经 stdout 返回。
- 修复宠物精灵主体未声明为可拖动区导致窗口无法拖动的问题：`-webkit-app-region` 不继承，精灵视口与精灵图现显式声明为拖动区。
- 修复宠物问题组下滑后跳回顶部、无法继续选择的问题：选项变更改为局部同步，快照刷新也会保留问题组与任务列表的滚动位置。
- 宠物问题组的选项改为与 Pi `question` 工具一致的 `1. 2. 3.` 编号显示，自由输入仍为常驻输入框。
- 新增“鼠标穿透”开关并默认关闭：关闭时 Electron 窗口直接接收鼠标输入，宠物原生拖动可用；开启时保留透明窗口的动态点击穿透。
- 修复宠物因预留上方任务气泡空间而无法贴近屏幕顶部的问题：窗口位于显示器上半区时宠物自动贴顶，任务气泡和普通通知改为向下展示；位于下半区时保持宠物贴底、气泡向上展示。
- 修复开启“鼠标穿透”后任务气泡和问题/审批面板也被穿透、无法代替会话操作的问题：穿透现在只作用于宠物主体，四色灯、工具栏和交互面板始终恢复鼠标命中。
- 点击四色灯浮出的任务气泡现在 5 秒后自动收起；指针停留在气泡上时暂停计时，移开后重新计时。
- 四色灯下的状态文字默认不再显示（四色灯已表达状态）；「显示状态」开关改用 `showStatusLabel` 字段保存，旧的默认开启值一次性失效，需要文字的用户可在设置中自行开启。
- 会话侧不再叠加桌面宠物E的问题与审批弹窗面板：终端内由各 CLI 自身的交互界面回答，宠物侧保留完整问题组入口，终端 Tab 继续显示待处理提醒。

...
- **原生供应商管理 Phase 1**：CLI-Manager 新增 Claude Code、Codex、Grok 原生供应商目录与 SQLite 数据层，支持供应商 CRUD、通用配置继承、JSON/TOML 校验与有效配置预览；单个供应商可保存多个明文 Key，并通过手动启用、脱敏展示和替代 Key 删除规则管理。当前设置页不再依赖 cc-switch 才能维护供应商，全局 Home 写入、项目/Worktree 原生切换和 CCS 导入将在后续阶段接入。
...
- **原生供应商数据库迁移兼容**：保留已发布的 v25 旧供应商迁移指纹，将 CLI-Manager 原生供应商表迁移到 v26；已有数据库升级后可正常打开供应商页，不再因缺少 `managed_*` 表显示 `provider_database_query_failed`。
- **Pi Hook 卸载状态**：删除 CLI-Manager 自有 Pi 扩展后，Hook 状态不再错误保留为已安装或部分安装。

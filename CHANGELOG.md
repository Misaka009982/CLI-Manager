...
- 历史会话“对话”页的每条可见消息现复用“原文”页操作栏：复制始终可用；本地可编辑消息可编辑、在后插入和删除。编辑/插入经既有确认获准后自动切换到“原文”页并打开同一原始消息的表单；SSH 与收藏快照继续只读。

### 文件预览

- 修复 TypeScript `.ts` 文件被错误识别为视频而无法预览的问题；前端、本地文件读取与 SSH Agent 现在统一将其交给既有文本解码链路，其他视频扩展名仍会在读取前拒绝。
